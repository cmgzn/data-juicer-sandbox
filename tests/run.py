...
parser.add_argument("--tag", choices=["standalone", "ray"], default="standalone", help="the tag of tests being run")
...
parser.add_argument(
    ...
    default=False,
    type=bool,
    help="Whether this unittest is conducted on a forked repo " "instead of a branch in the Data-Juicer repo.",
)
...
parser.add_argument(
    ...
    default=False,
    type=bool,
    help="whether to clear the downloaded models for tests. " "It's False in default.",
)
    ...
    for suite_discovered in discover:
        ...
        for test_suite in suite_discovered:
            ...
            for test_case in test_suite:
                logger.info(f"Add test case [{test_case._testMethodName}]" f" from {test_case.__class__.__name__}")
...
def main():
    ...
    test_suite = gather_test_cases(os.path.abspath(args.test_dir), args.pattern, args.tag, args.mode)
