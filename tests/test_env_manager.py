...
from data_juicer_sandbox.env_manager import CondaEnv, VirtualEnv
...
class TestCondaEnv(DataJuicerTestCaseBase):
    ...
    @patch("data_juicer_sandbox.env_manager.shutil.which", return_value="/usr/bin/conda")
    ...
    @patch("data_juicer_sandbox.env_manager.shutil.which", return_value=None)
    def test_check_availability_failed(self, mock_which):
        with self.assertRaises(ValueError):
            CondaEnv(env_name="dummy").check_availability()

    @patch("data_juicer_sandbox.env_manager.shutil.which", return_value="/usr/bin/conda")
    @patch("data_juicer_sandbox.env_manager.subprocess.run")
    def test_create_env_but_exists(self, mock_run, mock_which):
    ...
    @patch("data_juicer_sandbox.env_manager.shutil.which", return_value="/usr/bin/conda")
    @patch("data_juicer_sandbox.env_manager.subprocess.run")
    def test_create_new_env_from_name(self, mock_run, mock_which):
    ...
    @patch("data_juicer_sandbox.env_manager.shutil.which", return_value="/usr/bin/conda")
    @patch("data_juicer_sandbox.env_manager.subprocess.run")
    def test_create_new_env_from_config_path(self, mock_run, mock_which):
        mock_run.return_value.returncode = 0
        env = CondaEnv(env_config_path=self.config_path)
        env.create()
        # called in exists and called once when creating this new env
    ...
    @patch("data_juicer_sandbox.env_manager.shutil.which", return_value="/usr/bin/conda")
    @patch("data_juicer_sandbox.env_manager.subprocess.run")
    def test_create_new_env_failed(self, mock_run, mock_which):
    ...
    @patch("data_juicer_sandbox.env_manager.shutil.which", return_value="/usr/bin/conda")
    @patch("data_juicer_sandbox.env_manager.subprocess.run")
    def test_exists_false(self, mock_run, mock_which):
    ...
    @patch("data_juicer_sandbox.env_manager.shutil.which", return_value="/usr/bin/conda")
    @patch("data_juicer_sandbox.env_manager.subprocess.run")
    def test_install_deps_from_requirements_file(self, mock_run, mock_which):
        mock_run.return_value.returncode = 0
        env = CondaEnv(env_name=self.env_name)
    ...
    @patch("data_juicer_sandbox.env_manager.shutil.which", return_value="/usr/bin/conda")
    @patch("data_juicer_sandbox.env_manager.subprocess.run")
    def test_install_deps_from_lib_home(self, mock_run, mock_which):
        mock_run.return_value.returncode = 0
        env = CondaEnv(env_name=self.env_name)
    ...
    @patch("data_juicer_sandbox.env_manager.shutil.which", return_value="/usr/bin/conda")
    @patch("data_juicer_sandbox.env_manager.subprocess.run")
    def test_install_deps_from_non_existing_path(self, mock_run, mock_which):
    ...
    @patch("data_juicer_sandbox.env_manager.shutil.which", return_value="/usr/bin/conda")
    @patch("data_juicer_sandbox.env_manager.subprocess.run")
    def test_install_deps_from_list(self, mock_run, mock_which):
        mock_run.return_value.returncode = 0
        env = CondaEnv(env_name=self.env_name)
    ...
    @patch("data_juicer_sandbox.env_manager.shutil.which", return_value="/usr/bin/conda")
    @patch("data_juicer_sandbox.env_manager.subprocess.run")
    def test_install_deps_invalid(self, mock_run, mock_which):
    ...
    @patch("data_juicer_sandbox.env_manager.shutil.which", return_value="/usr/bin/conda")
    @patch("data_juicer_sandbox.env_manager.subprocess.run")
    def test_install_deps_failed(self, mock_run, mock_which):
    ...
    @patch("data_juicer_sandbox.env_manager.shutil.which", return_value="/usr/bin/conda")
    @patch("data_juicer_sandbox.env_manager.subprocess.run")
    def test_run_cmd(self, mock_run, mock_which):
        mock_run.return_value.returncode = 0
        env = CondaEnv(env_name=self.env_name)
